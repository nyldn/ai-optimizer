...
OUTPUT="$(
  AI_OPTIMIZER_RELEASE_BASE="file://$DIST_DIR" +  AI_OPTIMIZER_PREFIX="$INSTALL_ROOT" +  AI_OPTIMIZER_BIN_DIR="$BIN_DIR" +  PATH="/usr/bin:/bin" +  /bin/bash "$ROOT_DIR/install.sh" --version "$VERSION"
)"
...
AI_OPTIMIZER_DATA_DIR="$DATA_DIR" +AI_OPTIMIZER_LAUNCH_AGENTS_DIR="$AGENTS_DIR" +  "$BIN_DIR/ai-optimizer" setup --workspace-root "$TEST_ROOT/workspaces" >/dev/null

[ -f "$DATA_DIR/state-manifest.json" ]

AI_OPTIMIZER_PREFIX="$INSTALL_ROOT" +AI_OPTIMIZER_BIN_DIR="$BIN_DIR" +AI_OPTIMIZER_DATA_DIR="$DATA_DIR" +AI_OPTIMIZER_LAUNCH_AGENTS_DIR="$AGENTS_DIR" +  /bin/bash "$INSTALL_ROOT/scripts/uninstall.sh" --force >/dev/null
...
if AI_OPTIMIZER_RELEASE_BASE="file://$TAMPERED_DIR" +   AI_OPTIMIZER_PREFIX="$TEST_ROOT/tampered-install" +   AI_OPTIMIZER_BIN_DIR="$TEST_ROOT/tampered-bin" +   /bin/bash "$ROOT_DIR/install.sh" --version "$VERSION" >/dev/null 2>&1; then
